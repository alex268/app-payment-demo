...
public interface SaldoUpdateRepo extends CrudRepository<SaldoUpdate, SaldoUpdateKey> {
    ...
    Iterable<SaldoUpdate> listAllCreatedBefore(Collection<SaldoKey> keys, Instant ts);

    @Query(nativeQuery = true, value = ""
            + "DELETE FROM saldo_update "
            + "WHERE account_id = :accountId and part_num = :partNum AND created_ts < :ts "
            + "RETURNING amount"
    )
    Iterable<BigDecimal> deleteAndSumByIdBefore(Long accountId, Integer partNum, Instant ts);
}
